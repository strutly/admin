...
template.defaults.imports.distance = function (date) {
    if (date < 1000)
        return parseInt(date) + "米";
    return (Math.round(date / 100) / 10).toFixed(1) + "公里"
};
...
template.defaults.imports.escapeHtml4 = function (str) {
    if (null == str || '' == str) return "";
    return str.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/\\/g, "&quot;");
};

template.defaults.imports.htmlspecialchars = function (str) {
    if (null == str || '' == str) return "";
    return str.replace(/&amp;/g, "&").replace(/&lt;/g, "<").replace(/&gt;/g, ">").replace(/&quot;/g, "\\");
};
...
template.defaults.imports.split = function (date, format) {
    if (undefined == date || '' == date || null == date) return [];
